...
class LogInViewController: UIViewController, UITextFieldDelegate {
    ...
    private lazy var warningLabel: UILabel = {
        let label = UILabel()
        label.text = "Please enter 6 or more symbol"
    ...
    @objc private func didTapLogInButton() {
        ...
        if(email == false) {
            let alert = UIAlertController(title: "Warning !", message: "Incorrect email", preferredStyle: UIAlertController.Style.alert)
        ...
        if self.infoTextField.text == "" {
            ...
            let alert = UIAlertController(title: "Warning!", message: "Incorrectly, please try again", preferredStyle: UIAlertController.Style.alert)
            alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
                }
    }
